...
■ 화면  [오늘] / [달력]  (왼쪽 위에서 전환)
  ...
  달력
    헤더의 [주말] 을 누르면 주말 칸을 켜고 끕니다.
    끄면 칸이 5개로 넓어져 제목이 더 많이 보입니다. 이때 주말이 마감인 항목은
    앞 영업일 칸에 "5일(토)" 처럼 실제 날짜와 함께 얹힙니다.
    "마감이 있는 일" 만 표시합니다 (반복 업무는 넣지 않습니다 -
    되풀이되는 항목이 칸을 다 차지해서 정작 마감이 안 보이게 됩니다).
    색: 진한색 = 지남,  중간색 = 오늘,  연한색 = 예정,  테두리만 = 완료
    빈 칸을 누르면 그 날짜로 마감을 추가하고, 칩을 누르면 수정됩니다.
    한 칸에 3건까지 보이고 넘으면 "+N건 더" 로 그 날 전체를 볼 수 있습니다.
    ‹ › 또는 좌우 화살표 키로 달을 넘깁니다.

  오늘
    한 줄에 항목 하나입니다. 제목은 왼쪽, 마감 정보는 오른쪽에 붙습니다.
    오른쪽 칸들의 높이는 항목 수에 맞춰 딱 떨어지게 잡힙니다. 자리가 모자라면
    아래쪽 칸(메모)부터 줄어들고, 넘치는 것은 그 칸 안에서 스크롤됩니다.
